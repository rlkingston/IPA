module local_regression

! Based on the original Fortran 77 subroutines authored by W. S. Cleveland (Bell labs) and deposited at NetLib (https://www.netlib.org/)
! Some minimal changes incorporated by Richard Kingston (University of Auckland) to achieve Fortran 90 compatability and C/C++ interoperability 

  use,intrinsic :: iso_c_binding
  implicit none

!*
!*        Original Fortran 77 authored by 
!*        W. S. Cleveland
!*        Bell Laboratories
!* 
...
!* 
  contains
      ...
      subroutine lowess(x, y, n, f, nsteps, delta, ys, rw, res) bind (c)
      ...
      integer n
      integer nsteps
      real (c_float) :: x(n), y(n), f, delta, ys(n), rw(n)
      real (c_float) :: res(n)
      integer nright, min0, max0, i, j, ifix
      integer iter, last, m1, m2, ns, nleft
      real (c_float) :: abs, cut, cmad, r, d1, d2
      real (c_float) :: c1, c9, alpha, denom, float
      logical (c_bool) :: ok, userw
...
! radius will not decrease by move right
               ...
               goto  4
! fitted value at x(i)
            if (iter .gt. 1) userw = .true. 
   5        call lowest(x, y, n, x(i), ys(i), nleft, nright, res, userw, rw, ok)
            if (.not. ok) ys(i) = y(i)
...
! near 1, avoid underflow
  ...
  22  return
      ...
      subroutine lowest(x, y, n, xs, ys, nleft, nright, w, userw, rw, ok) bind(c)
      ...
      real (c_float) :: x(n), y(n), xs, ys, w(n), rw(n)
      logical (c_bool) :: userw, ok
      integer :: nrt, j
      real (c_float) :: abs, a, b, c, h, r
      real (c_float) :: h1, sqrt, h9, amax1, range
